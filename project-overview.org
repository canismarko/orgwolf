...
** High (crucial for any level of functionality)
   - Django models
   - Org-mode importer plugin
   - View (Basic HTML) for TODO list
   - view (Basic HTML) for daily agenda
...
** GTD
   ...
   - class Node:
     - get_children()
     - contact = ManyToMany(OtherPerson)
     - scope = ManyToMany(Scope)
     - text
     - parent
   ...
   - class ActionNode(Node):
     - (what-is-it criteria)
       - todo_state = ForeignKey(TodoState)
     - (scheduling criteria)
       - deadline
       - closed_date
       - scheduled
       - repeating
	 - repeat_strict (repeat from when completed or when scheduled)
     - (selection criteria)
       - mental_energy (high/low)
       - priority (A/B/C)
       - time_available (high/low)
       - tools_required = ManyToMany(Tools)
	 - (should "tools" really be "items"?)
       - location_required = ForeignKey(Location)
