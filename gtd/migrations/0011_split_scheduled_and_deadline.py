...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for node in orm.Node.objects.all():
            # Break the scheduled and deadline fields into separate date and time fields
            if node.scheduled:
                node.scheduled_date = node.scheduled.date()
                if node.scheduled_time_specific:
                    node.scheduled_time = node.scheduled.time()
            if node.deadline:
                node.deadline_date = node.deadline.date()
                if node.deadline_time_specific:
                    node.deadline_time = node.deadline.time()
            node.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for node in orm.Node.objects.all():
            # Combine the individual date and time fields into datetime for scheduled and deadline
            # Scheduled first
            if node.scheduled_time is None:
                new_time = dt.time()
                node.scheduled_time_specific = False
            else:
                new_time = node.scheduled_time
                node.scheduled_time_specific = True
            node.scheduled = dt.datetime.combine(node.scheduled_date,
                                                 new_time)
            # Deadline
            if node.deadline_time is None:
                new_time = dt.time()
                node.deadline_time_specific = False
            else:
                new_time = node.deadline_time
                node.deadline_time_specific = True
            node.deadline = dt.datetime.combine(node.deadline_date,
                                                new_time)
            node.save()
